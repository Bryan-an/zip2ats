/**
 * Nota de Débito (Debit Note) parser
 */
...
interface NotaDebitoParseResult {
  success: boolean;
...
/**
 * Parse a Nota de Débito XML document into normalized format
 */
export function parseNotaDebito(
  xml: NotaDebitoXML,
  autorizacion: AutorizacionInfo,
  xmlHash: string
): NotaDebitoParseResult {
  const errors: ValidationError[] = [];
  const warnings: ValidationError[] = [];

  try {
    const { notaDebito } = xml;
    const { infoTributaria, infoNotaDebito } = notaDebito;

    // Extract tax totals
    const totalesImpuesto = ensureArray(
      infoNotaDebito.totalConImpuestos.totalImpuesto
    );
    ...
    const document: ParsedDocument = {
      tipo: DOCUMENT_TYPES.NOTA_DEBITO,
      ...
      receptor: {
        tipoIdentificacion: infoNotaDebito.tipoIdentificacionComprador,
        identificacion: normalizeString(infoNotaDebito.identificacionComprador),
        razonSocial: normalizeString(infoNotaDebito.razonSocialComprador),
      },

      fecha: parseSRIDate(infoNotaDebito.fechaEmision),

      valores: {
        subtotal: parseMoneyToCents(infoNotaDebito.totalSinImpuestos),
        iva0,
        iva12,
        iva15,
        iva: ivaTotal,
        ...
        irbpnr: irbpnrTotal,
        propina: 0,
        total: parseMoneyToCents(infoNotaDebito.valorTotal),
      },
    ...
    if (!infoNotaDebito.numDocModificado) {
      warnings.push({
      ...
      errors: [
        {
          ...
          message: `Error al parsear nota de débito: ${error instanceof Error ? error.message : String(error)}`,
        },
      ],
    };
  }
